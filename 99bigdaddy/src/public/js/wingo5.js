...
function loadWingoHistory(targetIndex = 0, offset = 0, silent = false) {
  return GameHistoryClient.post({
    key: `wingo-5-history-${targetIndex}`,
    silent,
...
setInterval(() => {
  if (pageno === 0 && GameHistoryClient.shouldPollRound(5)) loadWingoHistory(0, 0, true);
}, 2000);
if (socket) {
          ...
          success: function (response) {
            ...
            if (firstGame && list_orders.length && String(firstGame.stage) === String(latestSettledPeriod) && Number(firstGame.status) !== 0) {
              ...
              if (firstGame.result == 0) {
                color = "Red+Violet";
                lottery_result.innerHTML = `<span>Lottery Result:</span>
              <span class='btn-boox' style="background: linear-gradient(to bottom right, #ff0000 50%, #ea3af0 50%);">${color}</span>
              <span class='btn-boox' style="background: linear-gradient(to bottom right, #ff0000 50%, #ea3af0 50%);">${firstGame.result}</span>
              <span class='btn-boox' style="background: linear-gradient(to bottom right, #ff0000 50%, #ea3af0 50%);">${type}</span>`;
              } else if (firstGame.result == 5) {
                color = "Green+Violet";
                lottery_result.innerHTML = `<span>Lottery Result:</span>
              <span class='btn-boox' style="background: linear-gradient(to bottom right, #20d90a 50%, #ea3af0 50%);">${color}</span>
              <span class='btn-boox' style="background: linear-gradient(to bottom right, #20d90a 50%, #ea3af0 50%);">${firstGame.result}</span>
              <span class='btn-boox' style="background: linear-gradient(to bottom right, #20d90a 50%, #ea3af0 50%);">${type}</span>`;
              } else if (firstGame.result % 2 == 0) {
                color = "Red";
                lottery_result.innerHTML = `<span style="font-size: 13px;">Lottery Result:</span>
              <span class='btn-boox' style="background: linear-gradient(to bottom, #ff0000, #ff0000); font-size: 12px;">${color}</span>
              <span class='btn-boox' style="background: linear-gradient(to bottom, #ff0000, #ff0000); font-size: 12px;">${firstGame.result}</span>
              <span class='btn-boox' style="background: linear-gradient(to bottom, #ff0000, #ff0000); font-size: 12px;">${type}</span>`;
              } else {
                color = "Green";
                lottery_result.innerHTML = `<span style="font-size: 13px;">Lottery Result:</span>
              <span class='btn-boox' style="background: linear-gradient(to bottom, #20d90a, #aae69d); font-size: 12px;">${color}</span>
