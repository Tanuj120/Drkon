...
const initiateUPIPayment = async (req, res) => {
    ...
    try {
        ...
        let recharge;
        try {
            recharge = await rechargeTable.create(newRecharge);
        } catch (error) {
            console.log("error in recharge: ", error);
        }

        // console.log("lo recharge bhi ho gya: ", recharge);
...
const verifyUPIPayment = async (req, res) => {
    ...
    const timeNow = new Date().toISOString();
    console.log("verify me bhi data aa gya", auth);
...
const addUserAccountBalance = async ({ money, phone }) => {
    try {
        ...
        if (free_bonus >= tenPercent) {
            money += tenPercent;
            await connection.query('UPDATE users SET free_bonus = free_bonus - ? WHERE phone = ?', [tenPercent, phone]);
        } else {
            money += free_bonus;
        ...
        await connection.query('UPDATE users SET money = money + ?, total_money = total_money + ? WHERE phone = ?', [adjustedMoney, adjustedMoney, phone]);

        console.log(`User's account updated successfully with ${adjustedMoney}`);
    } catch (error) {
