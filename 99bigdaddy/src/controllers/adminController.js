...
const getPhoneByInvite = async (req, res) => {
    const invite = req.params.invite;

    try {
        const [rows] = await connection.execute('SELECT phone FROM users WHERE code = ?', [invite]);

        if (rows.length > 0) {
            res.json({ phone: rows[0].phone });
        } else {
            res.status(404).json({ error: 'No user found with the given invite' });
        }
    } catch (error) {
...
const getUserByCode = async (req, res) => {
    ...
    try {
        const [countRows] = await connection.execute('SELECT COUNT(*) AS userCount FROM users WHERE invite = ?', [code]);

        if (countRows.length > 0) {
            res.json({ userCount: countRows[0].userCount });
        } else {
            res.status(404).json({ error: 'No user has this invite code' });
        }
    } catch (error) {
        res.status(500).json({ error: 'Internal Server Error' });
    }
...
const getReferredUsers = async (req, res) => {
    ...
    try {
        ...
        if (rows.length > 0) {
            ...
            res.status(404).json({ error: 'No referred users found' });
        }
    } catch (error) {
        res.status(500).json({ error: 'Internal Server Error' });
    }
...
const handlWithdraw = async (req, res) => {
    ...
    let id = req.body.id;
    let type = req.body.type;
    let remark = req.body.remark; // Get the remark from the request body
    if (!auth || !id || !type) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
    ...
    if (type == 'delete') {
        await connection.query(`UPDATE withdraw SET status = 2, remark = ? WHERE id = ?`, [remark, id]); // Update the status and remark
        const [info] = await connection.query(`SELECT * FROM withdraw WHERE id = ?`, [id]);
